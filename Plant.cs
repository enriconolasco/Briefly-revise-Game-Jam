...
public class Plant : Player2DController
{
    ...
    {
        if(hp <= 0 && !canShootSpecial)
        {
            if (GameObject.FindObjectOfType<plantSpecial>() != null)
            {
    ...
    public override void SpecialMove()
    {
        if ((Input.GetKeyDown(KeyCode.K) && isPlayer1) || (Input.GetKeyDown(KeyCode.S) && !isPlayer1))
        {
            if (canShootSpecial && facingForward)
            {
                ...
                GameObject proj = Instantiate(specialProjectile, new Vector3(transform.position.x + 0.3f, transform.position.y, transform.position.z), Quaternion.identity);
                proj.GetComponent<plantSpecial>().player = this.gameObject;
                specialProjectile = proj;
