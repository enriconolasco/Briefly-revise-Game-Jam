...
public class Player2DController : MonoBehaviour
{
    ...
    public float parryTimer = 3.0f;
    public float parryMissInactionTime = 3.0f;
    ...
    public virtual void Update()
    {
        ...
        UpdateHPText();
        UpdateAmmoText();
        if (numberOfJumps < maxJumps && guard != null)
        {
            Destroy(guard);
        }
        Die();
        Move();
        Jump();
        if (parryTimer >= parryMissInactionTime)
        {
    ...
    {
        if (Input.GetKey(KeyCode.V) && Input.GetKeyDown(KeyCode.E) && !isPlayer1 || Input.GetKey(KeyCode.DownArrow) && Input.GetKeyDown(KeyCode.O) && isPlayer1)
        {
            isParrying = true;
    ...
    public virtual void ParryTimer()
    {
    ...
    {
        if (parryTimer >= parryMissInactionTime && numberOfJumps == maxJumps)
        {
    ...
    public virtual void Recharge()
    {
        ...
        if ((Input.GetKey(KeyCode.L) && isPlayer1)|| (Input.GetKey(KeyCode.D) && !isPlayer1))
        {
            if (ammo == 0)
            {
    ...
    {
        if (col.gameObject.GetComponent<TranscendenceScripts>())
        {
            hp = hp - 1.5f;
        }
